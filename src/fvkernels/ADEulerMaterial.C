...
#include "ADEulerMaterial.h"

registerMooseObject("SquirrelApp", ADEulerMaterial);

InputParameters
ADEulerMaterial::validParams()
{
  ...
  params.addRequiredCoupledVar("temp", "The name of the temperature variable");
  params.addParam<Real>("rgas", 1.0, "Gas constant R");
  ...
  params.addParam<MaterialPropertyName>(
      "rho_T", "rho_T", "The product of the density and the temperature");
  params.addParam<MaterialPropertyName>(
      "pressure", "pressure", "The name of the pressure material property to create");
  return params;
}

ADEulerMaterial::ADEulerMaterial(const InputParameters & parameters)
  : Material(parameters),
    ...
    _rho_T(declareADProperty<Real>(getParam<MaterialPropertyName>("rho_T"))),
    _pressure(declareADProperty<Real>(getParam<MaterialPropertyName>("pressure")))
{
}

void
ADEulerMaterial::computeQpProperties()
{
  ...
  _rho_T[_qp] = _rho[_qp] * _temp[_qp];
  _pressure[_qp] = _rgas * _rho[_qp] * (_temp[_qp] + 273.15);
}
