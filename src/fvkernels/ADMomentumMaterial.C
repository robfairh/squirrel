...
#include "ADMomentumMaterial.h"

registerMooseObject("SquirrelApp", ADMomentumMaterial);

InputParameters
ADMomentumMaterial::validParams()
{
  ...
  params.addRequiredCoupledVar("energy", "The name of the energy variable");
  ...
  params.addParam<MaterialPropertyName>(
      "pres", "pres", "The name of the pressure material property to create");
  return params;
}

ADMomentumMaterial::ADMomentumMaterial(const InputParameters & parameters)
  : Material(parameters),
    ...
    _pres(declareADProperty<Real>(getParam<MaterialPropertyName>("pres"))),
...
void
ADMomentumMaterial::computeQpProperties()
{
  ...
  // _pres[_qp] = 0;
  _pres[_qp] = 0.01 * (1.67-1)*(_energy[_qp] - 0.5 * _rho[_qp] * _vel_x[_qp] * _vel_x[_qp]);
